...
> Dashboard-Builder - Open source tool by Qyroxen

`cli` `dashboard` `developer-tools` `golang` `visualization`

## What is it?

**Dashboard Builder** is a cli tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
```bash
# Basic usage
./Dashboard-Builder --path ./target

# With options
./Dashboard-Builder --path ./target --format json --output report.json

# Verbose mode
./Dashboard-Builder --path ./target --verbose
```
...
### Basic scan
```bash
./Dashboard-Builder --path ./my-project
```

### JSON report
```bash
./Dashboard-Builder --path ./my-project --format json --output report.json
```
...
- name: Run Dashboard Builder
  run: ./Dashboard-Builder --path . --format json --output report.json
```
